...
export default function LoMasNuevoPage({
  products,
  categories,
  attributes,
}: LoMasNuevoPageProps) {
  ...
  // sortBy siempre es "newest" para esta página
  ...
  const filteredProducts = useMemo(() => {
        ...
        if (isAttribute) {
          result = result.filter((product) => {
            ...
            const productAttr = product.attributes?.find((attr) => {
              ...
              // Debug temporal
              if (key === "pa_color" || keyName.includes("color")) {
                console.log("Debug color filter:", {
                  key,
                  keyName,
                  attrName,
            ...
            const hasMatch = value.some((selectedValue) =>
              productAttr.options?.includes(selectedValue)
            );

            // Debug temporal
            if (key === "pa_color" || key.toLowerCase().includes("color")) {
              console.log("Debug color match:", {
                productName: product.name,
                selectedValues: value,
                attrOptions: productAttr.options,
                hasMatch,
              });
            }
    ...
    // Ordenamiento (siempre por fecha para productos nuevos)
    // Los productos ya vienen ordenados por fecha del servidor
    // No se necesita sort adicional
  ...
  // Función para obtener la categoría principal de un producto (no se usa actualmente)
  // const getMainCategory = (product: IProduct) => {
  //   if (!product.categories || product.categories.length === 0) return null;
  //
  //   // Buscar categoría raíz (parent === 0)
  //   const rootCategory = product.categories.find(
  //     (cat) => (cat as ICategory & { parent?: number }).parent === 0
  //   );
  //
  //   if (rootCategory) return rootCategory;
  //
  //   // Si no hay categoría raíz, usar la primera
