...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${apiUrl}/wp-json/izipay/v1/validate`;
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Izipay validation API error: ${response.status} - ${errorData}`
      );
    }
    ...
    console.error("Error validating Izipay signature:", error);
    return NextResponse.json(
      { error: "Error validating Izipay signature" },
      { status: 500 }
    );
  }
}
